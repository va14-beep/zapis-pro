  ...
  document.getElementById('app').innerHTML = `
    <div class="min-h-screen">
      <header class="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <button onclick="renderProfile()" class="text-sm text-slate-600 hover:text-indigo-600 font-medium">Профиль</button>
      ...
      <main class="max-w-5xl mx-auto px-4 py-6 space-y-6 fade">
        ...
        ${user.isPaid ? `
        <div class="bg-green-50 border border-green-100 rounded-2xl p-4 text-sm text-green-800">
          Подписка активна
        </div>` : ''}
...
function renderPaywall() {
  ...
  document.getElementById('app').innerHTML = `
    <div class="min-h-screen flex items-center justify-center px-4">
      <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-100 p-8 text-center fade">
        ...
        <div class="bg-slate-50 rounded-xl p-5 mb-6">
          ...
          <div class="flex justify-center mb-3">
            <img src="/qr-payment.png" alt="QR для оплаты" class="w-48 h-48 rounded-lg border border-slate-200">
          </div>
    ...
    <div class="min-h-screen bg-slate-50">
      <header class="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <button onclick="renderDashboard()" class="text-sm text-slate-600 hover:text-indigo-600 font-medium">← Кабинет</button>
            <button onclick="logout()" class="text-sm text-slate-500 hover:text-red-600">Выйти</button>
          </div>
        </div>
      </header>
      ...
      <main class="max-w-3xl mx-auto px-4 py-8 space-y-6 fade">
        <div>
          ...
          <p class="text-slate-500 mt-1">Ваши данные и достижения</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Имя / название</label>
              <input id="prof-name" value="${escapeHtml(user.name || '')}" class="w-full border border-slate-200 rounded-xl px-4 py-2.5">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Email</label>
              <input value="${escapeHtml(user.email || '')}" disabled class="w-full border border-slate-200 rounded-xl px-4 py-2.5 bg-slate-50 text-slate-500">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Телефон</label>
              <input id="prof-phone" value="${escapeHtml(user.phone || '')}" class="w-full border border-slate-200 rounded-xl px-4 py-2.5" placeholder="+7 ...">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Telegram Chat ID</label>
              <input id="prof-tg" value="${escapeHtml(user.telegramChatId || '')}" class="w-full border border-slate-200 rounded-xl px-4 py-2.5" placeholder="Например 123456789">
              <p class="text-xs text-slate-500 mt-1">Чтобы получать уведомления о новых записях. Как узнать ID — написано ниже.</p>
            </div>
            ...
            <div class="mt-4 p-4 bg-slate-50 rounded-xl text-sm text-slate-600">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="bg-white rounded-2xl border border-slate-100 p-4 text-center">
            ...
            <div class="text-xs text-slate-500 mt-1">Всего записей</div>
          </div>
          <div class="bg-white rounded-2xl border border-slate-100 p-4 text-center">
            <div class="text-2xl font-bold text-green-600">${paid}</div>
            <div class="text-xs text-slate-500 mt-1">Оплачено</div>
          </div>
          <div class="bg-white rounded-2xl border border-slate-100 p-4 text-center">
            <div class="text-2xl font-bold text-indigo-600">${clients}</div>
            <div class="text-xs text-slate-500 mt-1">Клиентов</div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-sm text-slate-500">${doneCount} из ${achievements.length}</span>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ${achievements.map(a => `
              <div class="flex items-start gap-3 p-3 rounded-xl border ${a.done ? 'border-indigo-100 bg-indigo-50/50' : 'border-slate-100 bg-slate-50 opacity-60'}">
                <div class="text-2xl">${a.icon}</div>
                <div>
                  <div class="font-medium text-sm ${a.done ? 'text-slate-900' : 'text-slate-500'}">${a.title}</div>
              ...
              masters.map(m => {
                ...
                <div class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <div class="text-sm text-slate-500">${escapeHtml(m.email)} · ${m.bookingsCount} записей</div>
                  </div>
                  <div class="flex gap-2">
