...
app.post('/api/register', async (req, res) => {
  ...
  const master = {
    ...
    phone: phone || '',
    password: hash,
    slug: Math.random().toString(36).slice(2, 8), // короткий код типа a7k2m9
...
app.get('/api/me', authMiddleware, (req, res) => {
  ...
  res.json({
    ...
    hasAccess: hasAccess,
    telegramChatId: master.telegramChatId || ''
  });
});
...
app.put('/api/me', authMiddleware, (req, res) => {
  ...
  db.get('masters').find({ id: req.user.id }).assign(updates).write();
  const master = db.get('masters').find({ id: req.user.id }).value();
  res.json({
    id: master.id,
    name: master.name,
...
Откройте кабинет, чтобы подтвердить.`;
    sendTelegram(master.telegramChatId, msg);
  }
...
app.patch('/api/admin/masters/:id', authMiddleware, adminMiddleware, (req, res) => {
  const { active, isPaid, endTrial } = req.body;
  const updates = {};
  if (typeof active !== 'undefined') updates.active = !!active;
  if (typeof isPaid !== 'undefined') updates.isPaid = !!isPaid;
  if (endTrial) {
    // Ставим дату окончания на вчера
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    updates.trialEndsAt = yesterday.toISOString();
...
app.post('/api/telegram/webhook', express.json(), (req, res) => {
  try {
    ...
    if (text === '/start' || text.startsWith('/start')) {
      const reply = `👋 Привет! Это бот уведомлений <b>ЗаписьPRO</b>.

Я буду присылать тебе сообщения, когда клиенты записываются через твою ссылку.

<b>Как подключить уведомления:</b>

1. Твой Chat ID: <code>${chatId}</code>
2. Скопируй этот номер
3. Зайди на сайт → Профиль
4. Вставь Chat ID в поле «Telegram Chat ID»
5. Нажми «Сохранить»

Готово! После этого ты будешь получать уведомления о новых записях.

Если что-то не работает — напиши владельцу сервиса.`;
...
app.listen(PORT, () => {
  ...
  if (TELEGRAM_BOT_TOKEN && process.env.RAILWAY_PUBLIC_DOMAIN) {
    ...
    fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/setWebhook`, {
      ...
      .then(r => r.json())
      .then(d => console.log('Telegram webhook:', d.ok ? 'OK' : d))
